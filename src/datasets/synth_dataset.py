...
class SyntheticDataset(Dataset):
    ...
    def __init__(
        ...
        self.length = length
        self.volume = volume
        self.attr_pair_per_block = attr_pair_per_block
        assert self.attr_pair_per_block <= len(
            self.non_default_attribute_pairs
        ), "too much atributes requested"
        self.instance_transforms = None
    ...
    def __getitem__(self, ind):
        ...
        block_type_grid: torch.Tensor = torch.randint(
            low=0,
            high=len(self.idx2block),
            size=(self.volume, self.volume, self.volume),
            dtype=torch.long,
        )
        ...
        for i, (attr, values) in enumerate(self.non_default_attribute_pairs):
            head_key = get_head_key(attr, values)

            if i < self.attr_pair_per_block:
                mask = torch.ones_like(block_type_grid, dtype=torch.bool)
            else:
                mask = torch.zeros_like(block_type_grid, dtype=torch.bool)

            value = torch.randint_like(
                input=block_type_grid[mask], high=len(values), dtype=torch.long
            )

            attributes_values[head_key] = value
            attributes_masks[head_key] = mask
